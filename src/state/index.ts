...
import moveWithinBounds from '../helpers/moveWithinBounds.ts';
import getCoordsAroundCentre from '../helpers/getCoordsAroundCentre.ts';
...
interface BaseCell {
  ...
  belowCell?: 'MINE' | 'GOLD' | 'DOOR' | 'DEVIL_DOOR' | 'EMPTY';
}
...
interface DevilDoorCell extends BaseCell {
  belowCell: 'DEVIL_DOOR';
}

export type CellFragmentData =
  | GildedCell
  | DarkCell
  | MinedCell
  | ClickedCell
...
export type CellUpdateData = {
  [key: string]: GildedCell | DarkCell | MinedCell | ClickedCell | FlaggedCell | TorchedCell | DoorCell | DevilDoorCell;
};
    ...
    immer((set) => ({
          ...
          state.spawns.forEach((spawn) => {
            if (spawn.type === 'SNAKE') {
              ...
              while (!nextDestination) {
                nextDirection = chooseWeightedOption([
                  { id: 'N', weight: spawn.facing === 'N' ? 50 : 50 / 3 },
                  { id: 'E', weight: spawn.facing === 'E' ? 50 : 50 / 3 },
                  { id: 'S', weight: spawn.facing === 'S' ? 50 : 50 / 3 },
                  { id: 'W', weight: spawn.facing === 'W' ? 50 : 50 / 3 },
              ...
              if (spawn.clicksSinceActive >= 2) {
                // activate its ability, which for now is just covering a 3x3 area behind it
                // and spawning a mine or two within that area
                // use its past position for now; in future can change it to be more dynamic.
                ...
                spawn.clicksSinceActive++;
              }

              spawn.position = [nextDestination.x, nextDestination.y];
              spawn.facing = nextDirection;
            }
          });
        }),
        ...
        set((state) => {
          ...
          item.effects.forEach((effect) => {
            if (effect.effectType === 'ADDITIVE') {
              // @ts-expect-error never nonsense
        ...
        set((state) => {
          ...
          if (state.shopping) {
            ...
            const darknesses = calculateDarknessLevels(
              [
                { coordinate: `${state.position[0]}:${state.position[1]}:${0}`, strength: 6 },
      ...
      placeTorch: (c: Coordinate) =>
        set((state) => {
          ...
          const darknesses = calculateDarknessLevels(
            [
              { coordinate: `${state.position[0]}:${state.position[1]}:${state.layer}`, strength: 6 },
              ...state.torchIndex[state.layer].map((t) => ({ coordinate: t, strength: 9 })),
            ],
            state.cellData,
            state.layer,
        ...
        set((state) => {
          ...
          const { objectResults, indices } = generateLayerObjectV2(
            [
              ...
              { name: 'DOOR', frequency: DOORS },
              { name: 'DEVIL_DOOR', frequency: DEVIL_DOORS },
            ],
            WIDTH,
            HEIGHT,
            state.layer,
